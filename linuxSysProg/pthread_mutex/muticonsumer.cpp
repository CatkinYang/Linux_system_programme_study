/*************************************************************************
 ...
 > Author: Catkin 
 > Created Time: Sat Mar  4 11:17:23 2023
 ************************************************************************/
...
pthread_mutex_t mutex1 = PTHREAD_MUTEX_INITIALIZER;      // 定义/初始化一个互斥量  
...
void *produser(void *arg)  {  
    while (1) {  
        ...
        pthread_mutex_lock(&mutex1);                         // 加锁 互斥量  
        mp->next = head;                                    // 写公共区域  
        head = mp;  
        pthread_mutex_unlock(&mutex1);                       // 解锁 互斥量  
...
void *consumer(void *arg)  {  
    while (1) {  
        ...
        pthread_mutex_lock(&mutex1);                         // 加锁 互斥量  
        if (head == NULL) {  
            pthread_cond_wait(&has_data, &mutex1);           // 阻塞等待条件变量, 解锁  
